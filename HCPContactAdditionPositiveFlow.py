...
FilePath = str(Path().resolve()) + '\Excel Files\HCPContactAddition.xlsx'
Seconds = 1
...
for RowIndex in range(2, RowCount + 1):
    ...
    time.sleep(Seconds)
    Element = BEP.WebElement(driver, EC.presence_of_element_located, By.XPATH, '//span[. = "Clinic Staff"]', 60)
    Element.click()
...
for RowIndex in range(2, RowCount + 1):
    ...
    # DHCmail
    time.sleep(Seconds)
    Element = BEP.WebElement(driver, EC.presence_of_element_located, By.XPATH, '//force-record-layout-text-area//textarea', 60)
    driver.execute_script('arguments[0].scrollIntoView(true);', Element)
    if (str(RWDE.ReadData(FilePath, Sheet, RowIndex, 9)) != 'None'):
        Element.send_keys(RWDE.ReadData(FilePath, Sheet, RowIndex, 9))
    else:
    ...
    if (ErrorElementAvailable == True):
        ...
        if(RWDE.ReadData(FilePath, Sheet, RowIndex, 21) == Element.text):
            RWDE.WriteData(FilePath, Sheet, RowIndex, 22, Element.text)
        ...
        #Mobile
        time.sleep(Seconds)
        Element = BEP.WebElement(driver, EC.presence_of_element_located, By.XPATH, '//force-record-layout-row[5]/slot/force-record-layout-item[2]//input', 60)
        driver.execute_script('arguments[0].scrollIntoView(true);', Element)
        ...
        #Mailing Street
        time.sleep(Seconds)
        Element = BEP.WebElement(driver, EC.presence_of_element_located, By.XPATH, '//div/lightning-textarea//textarea', 60)
        driver.execute_script('arguments[0].scrollIntoView(true);', Element)
